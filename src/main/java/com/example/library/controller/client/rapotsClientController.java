...
public class rapotsClientController {
    ...
    private void handleDelete() {
        ...
        if (result.isPresent() && result.get() == approveButton) {
            try (Connection conn = DatabaseConnection.getConnection()) {
                ...
                // 1️⃣ جلب بيانات العميل والإجمالي من الفاتورة
                String selectSQL = "SELECT customer_id, total FROM client_sales WHERE sale_id = ?";
                String customerId = null;
                BigDecimal totalAmount = BigDecimal.ZERO;

                try (PreparedStatement pstmt = conn.prepareStatement(selectSQL)) {
                    pstmt.setInt(1, currentSaleId);
                ...
                // 2️⃣ إذا وجد عميل، اطرح المبلغ من الدين
                if (customerId != null && totalAmount != null && totalAmount.compareTo(BigDecimal.ZERO) > 0) {
                    String updateDebtSQL = "UPDATE client_debts SET amount = amount - ? WHERE customer_id = ? AND status != 'مدفوع'";
                    try (PreparedStatement pstmt = conn.prepareStatement(updateDebtSQL)) {
                        pstmt.setBigDecimal(1, totalAmount);
                        pstmt.setString(2, customerId);
                        pstmt.executeUpdate();
                    }

                    // 3️⃣ إذا أصبح الدين صفر أو أقل، عدّل الحالة أو احذف السجل
                    String deleteOrUpdateSQL = "UPDATE client_debts SET amount = 0, status = 'مدفوع' WHERE customer_id = ? AND amount <= 0";
                    try (PreparedStatement pstmt = conn.prepareStatement(deleteOrUpdateSQL)) {
                ...
                showSuccessAlert("تم الحذف", "تم حذف الفاتورة وتحديث الدين بنجاح.");

                Stage stage = (Stage) deleteButton.getScene().getWindow();
                stage.close();

            } catch (SQLException e) {
                showFailedAlert("خطأ", "فشل في الحذف أو تحديث الديون.");
            }
        }
    }
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement stmt = conn.prepareStatement(
                        "DELETE FROM client_sales_item WHERE sale_id = ?")) {
                ...
                try (PreparedStatement stmt = conn.prepareStatement(
                        "DELETE FROM client_sales WHERE sale_id = ?")) {
                    stmt.setInt(1, saleId);
                    stmt.executeUpdate();
                }
