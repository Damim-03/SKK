...
public class printFormPurchasesController {
    ...
    private void printHighQualityNode(Node node, Stage ownerWindow) {
        ...
        if (job == null) {
            showFailedAlert("خطأ", "لا استطيع العثور على الطابعة, الرجاء التأكد من وصل الطابعة بالبرنامج.");
            return;
        }
        ...
        if (job.showPrintDialog(ownerWindow)) {
            try {
                ...
                if (job.printPage(layout, printContent)) {
                    ...
                    showWarningAlert("خطأ", "خطأ في الطباعة.");
                }
            } catch (Exception e) {
                showFailedAlert("فشل", "فشل الطباعة.");
            }
        }
    }
