...
public class raportsPurchaseController {
    ...
    public void setPurchaseData(Sale sale, ObservableList<SaleItem> saleItems) {
        if (sale == null) return;
    ...
    private void printHighQualityNode(Node node, Stage ownerWindow) {
        ...
        if (job == null) {
            showWarningAlert( "تنبيه", "لا يمكن العثور على الطابعة, الرجاء توصيل الطابعة بالبرنامج.");
            return;
        }
        ...
        if (job.showPrintDialog(ownerWindow)) {
            try {
                ...
                if (job.printPage(layout, printContent)) {
                    ...
                    showWarningAlert("خطأ", "فشلت الطباعة.");
                }
            } catch (Exception e) {
                showFailedAlert("فشل", "استثناء الطباعة.");
            }
        }
    }
